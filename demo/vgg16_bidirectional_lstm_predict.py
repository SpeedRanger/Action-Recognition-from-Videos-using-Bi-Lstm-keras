...
def main():
    ...
    # print([label for (label, label_index) in list(predictor.labels.items())[:10]])
    videos = scan_ucf_with_labels(data_dir_path, [label for (label, label_index) in list(predictor.labels.items())[:10]])
